...
#else
        var phaseCount = (int) (deltaNow.Ticks / (double) phasePeriod.Ticks);
        var standFrom = from + Multiple(every, phaseCount + 1) + Multiple(stand, phaseCount);
#endif
...
#else
        var sitFrom = from + Multiple(every, phaseCount) + Multiple(stand, phaseCount);
#endif
...
#if NETSTANDARD2_0
    private static TimeSpan Multiple(TimeSpan timeSpan, double factor)
    {
